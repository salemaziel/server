#!/usr/bin/env bash
# Inspired by https://github.com/complexorganizations/wireguard-manager
# Modified for more privacy-friendly setup
...
read -p "Verify DNS server on system? [y/n]  " "verify_dns"
case $verify_dns in
  y)
    echo "Verifying system's dns servers using Quad9 servers in 5 seconds (Ctrl+c to cancel"
    sleep 6
    validate-dns-server
    ;;
  n)
    echo "Skipping checking system for working DNS server"
    sleep 3
    ;;
esac
...
#        SERVER_HOST_V4="$(curl -4 -s 'https://api.ipengine.dev' | jq -r '.network.ip')"
        SERVER_HOST_V4="$(curl -4 -s 'https://ipv4.icanhazip.com')"
...
#         SERVER_HOST_V4="$(ip route get 9.9.9.9 | grep src | sed 's/.*src \(.* \)/\1/g' | cut -f1 -d ' ')"
#                 if [ -z "${SERVER_HOST_V4}" ]; then
...
# 	 read -rp "Custom IPv4: " -e -i "$(curl -4 -s 'https://ipv4.icanhazip.com')" SERVER_HOST_V4
#        if [ -z "${SERVER_HOST_V4}" ]; then
#          SERVER_HOST_V4="$(curl -4 -s 'https://ipv4.icanhazip.com')"
#        fi
#        ;;
...
#        SERVER_HOST_V6="$(curl -6 -s 'https://api.ipengine.dev' | jq -r '.network.ip')"
        SERVER_HOST_V6="$(curl -6 -s 'https://ipv6.icanhazip.com')"
...
#        read -rp "Custom IPv6: " -e -i "$(curl -6 -s 'https://ipv6.icanhazip.com')" SERVER_HOST_V6
        if [ -z "${SERVER_HOST_V6}" ]; then
#          SERVER_HOST_V6="$(curl -6 -s 'https://api.ipengine.dev' | jq -r '.network.ip')"
        SERVER_HOST_V6="$(curl -6 -s 'https://ipv6.icanhazip.com')"
        fi
  ...
  function custom-dns() {
    if [ -f "${WIREGUARD_INTERFACE}" ]; then
      if [[ ${CUSTOM_DNS} =~ ^[Yy]$ ]]; then
        ...
        echo "  1) Google"
        echo "  2) AdGuard"
        echo "  3) NextDNS (Recommended)"
        echo "  4) OpenDNS"
        echo "  5) Cloudflare"
        echo "  6) Verisign"
        echo "  7) Quad9 (Recommended)"
        echo "  8) FDN"
