...
[Comparison with Other Phones](#comparison)
-------------------------------------------
...
*   Attempting to hide kernel symbols via `kptr_restrict` ignores the fact that they're clearly visible in the `System.map` file on disk, [among](guides/linux-hardening.html#sysctl-kernel) [other](guides/linux-hardening.html#other-kernel-pointer-leaks) [sources](guides/linux-hardening.html#kernel-self-compilation).
...
[Hardware Kill Switches](#hardware-kill-switches)
-------------------------------------------------
...
[Modem Isolation](#modem-isolation)
-----------------------------------

Modem isolation isn't anything special. For example, [Qualcomm SoCs have isolated the modem via an IOMMU for years](https://i.blackhat.com/USA-19/Thursday/us-19-Pi-Exploiting-Qualcomm-WLAN-And-Modem-Over-The-Air-wp.pdf), among others. The unorthodox way in which the Librem 5 attempts to isolate the modem is via the Linux kernel USB stack, which is not a strong barrier, as shown in the [Linux article](linux.html).  
